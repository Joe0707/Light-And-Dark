...
namespace MOBAServer.Logic
{
    public class PlayerHandler:SingleSend,IOpHandler
    {
        ...
        /// 账号缓存
        /// </summary>
        public AccountCache accountCache = Caches.Account;
        /// <summary>
        /// 玩家缓存
        /// </summary>
        ...
        {
            //非法操作
            if (playerCache.GetId(client) != playerId) return;
            //获取匹配结果
        ...
        private void onAdd(MobaClient client, string addName)
        {
            ...
            foreach (var item in friends)
            {
                ...
                {
                    Send(client, OpCode.PlayerCode, OpPlayer.RequestAdd, -4, "该玩家已经是你的好友");
                    return;
                }
            }
            //如果能获取到数据模型 先判断他是否在线
            ...
            MobaClient tempClient = null;
                ...
                foreach (string item in friends)
                {
                    ...
                    if (!playerCache.isOnline(id))
                        continue;
                    tempClient = playerCache.GetClient(id);
                    Send(tempClient, OpCode.PlayerCode, OpPlayer.FriendOnline, 0, "此玩家已上线", model.Id);
                }
            }
            #endregion
